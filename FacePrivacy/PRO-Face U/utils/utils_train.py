...
def acc(threshold,predict,utility_factor):
    # 将余弦相似度张量与阈值比较，得到二元张量
    binary_predictions = (predict <= threshold).float() if utility_factor else (predict >= threshold).float()
    # 计算准确率
...
def pass_epoch_utility(embedder, utility_fc,noise_mk ,x_ori,optim2,obfuscator,gender_classifier, recognizer, dataloader, swap_target_set=(),
               cartoon_set=(), session='', dir_image='./images', dir_checkpoint='./checkpoints', optimizer=None,
               scheduler=None, show_running=True, writer=None, epoch=0, debug=False):
    """Train or evaluate over a data epoch.
    ...
    for i_batch, data_batch in enumerate(dataloader):
        ...
        # if i_batch % 2 == 0:
        #     np.random.seed(0)
        #     # 生成一个16x3x112x112的高斯噪声数组，均值为0，标准差为1
        #     xa_obfs = np.random.normal(loc=0.0, scale=1.0, size=(16, 3, 112, 112))
        #
        #     # 将numpy数组转换为torch.Tensor
        #     xa_obfs = torch.from_numpy(xa_obfs).float()
...
################################################################################################################
        ...
        utility_factor2= random.randint(0, 1)
        # condition_utility = torch.full((_bs, 1, _w // 2, _h // 2), torch.tensor(utility_factor).float()).to(device)
        utility_cond_init = torch.tensor([float(utility_factor), random.choice([1, 1 - float(utility_factor)])]).repeat(_bs, 1).to(device)
        ...
        # xa_adv = torch.clamp(xa_adv, min=0, max=1)
        # eta = torch.clamp(xa_adv - xa, min=-8/255, max=8/255)
        # xa_adv = torch.clamp(xa + eta, min=0, max=1)
        # xa_rev, xa_obfs_rev = embedder(password_a.repeat(1, 4, 1, 1), xa_proc, password_a, rev=True)

        # Feed random noise as input in recovery process
        noise = password_a.repeat(1, 3, 1, 1) if c.SECRET_KEY_AS_NOISE else gauss_noise((_bs, _c * 4, _w // 2, _h // 2)).to(device)
        xa_rev, xa_obfs_rev = embedder(noise, xa_adv, condition, rev=True)
        ...
        ## Three kinds of perceptual losses
        #loss_img_perc = lpips_loss(xa, xa_proc)
        # g_loss = guide_loss(xa_adv.cuda(), xa.cuda()).to(device)
        # l22_loss = torch.norm(xn, p=2)

        # loss_img_perc = lpips_loss(xa, xa_adv)
        # loss_img_l1 = l2_loss(xa, xa_adv)
        # # loss_img_l2=l1_loss(low1,low2)

        loss_img_perc0 = lpips_loss(xa_obfs, xa_adv)
        loss_img_l10 = l2_loss(xa_obfs, xa_adv)

        loss_img_perc = lpips_loss(xa, xa_adv)
        loss_img_l1 = l2_loss(xa, xa_adv)
        # loss_img_l2=l1_loss(low1,low2)
        loss_image = 6 * loss_img_perc0 + loss_img_l10 if utility_factor else 6 * loss_img_perc + loss_img_l1
        # perc_loss = vgg_loss(xa_adv,xa).to(device)
        # l_loss = guide_loss(low1.cuda(), low2.cuda()).to(device)
        #loss_img_l1 = l1_loss(xa, xa_proc)
        #loss_image = 5 * loss_img_perc + loss_img_l1

        ## adv losses
        #loss_img_l2 = l2_loss(xa_proc,xa_adv)
        #A = 8
        #loss_perturb = torch.mean(torch.norm(xa_out_z.view(xa_out_z.shape[0], -1), 2, dim=1))
        #loss_perturb = torch.max(loss_perturb - A, torch.zeros(1))

        # 沿指定维度计算Frobenius范数
        # loss_perturb = torch.mean(torch.norm(xa_out_z.view(xa_out_z.shape[0], -1), 2, dim=1))
        # loss_perturb = torch.mean(torch.max(loss_perturb - 8, torch.zeros(1, device=device)))
        ...
        if (i_batch % c.SAVE_IMAGE_INTERVAL == 0) or (i_batch == 1):
            save_image(normalize(xa),
                       f"{dir_image}/{mode}_ep{epoch}_batch{i_batch}_orig.jpg", nrow=4)
            save_image(normalize(xa_out_z),
                       f"{dir_image}/{mode}_ep{epoch}_batch{i_batch}_z.jpg", nrow=4)
            save_image(normalize(xa_adv),
                       f"{dir_image}/{utility_factor}-{mode}_ep{epoch}_batch{i_batch}_adv.jpg", nrow=4)
            save_image(normalize(xa_obfs),
                       f"{dir_image}/{mode}_ep{epoch}_batch{i_batch}_xn.jpg", nrow=4)
            # save_image(normalize(xa_obfs),
            #            f"{dir_image}/{mode}_ep{epoch}_batch{i_batch}_{obf_type}.jpg", nrow=4)
            save_image(normalize(xa_rev),
                       f"{dir_image}/{mode}_ep{epoch}_batch{i_batch}_rev.jpg", nrow=4)
        ...
        metrics_batch = {}
        for metric_name, metric_fn in metric_functions.items():
            metrics_batch[metric_name] = metric_fn(0.6,cos_loss(embedding_adv,embedding_orig),utility_factor)
            metrics[metric_name] = metrics.get(metric_name, 0) + metrics_batch[metric_name]
        # metrics['privBudget'] = metrics.get('privBudget', .0) + privacy_budget
        ...
        if writer is not None and embedder.training:
            if writer.iteration % writer.interval == 0:
                writer.add_scalars('loss_img_perc', {mode: loss_img_perc.detach().cpu()}, writer.iteration)
                # writer.add_scalars('loss_l', {mode: l_loss.detach().cpu()}, writer.iteration)
                writer.add_scalars('loss_rec', {mode: loss_rec.detach().cpu()}, writer.iteration)
                #writer.add_scalars('loss_rec_wrong', {mode: loss_rec_wrong.detach().cpu()}, writer.iteration)
                writer.add_scalars('loss_batch', {mode: loss_batch.detach().cpu()}, writer.iteration)
                writer.add_scalars('loss_utility', {mode: loss_utility.detach().cpu()}, writer.iteration)
